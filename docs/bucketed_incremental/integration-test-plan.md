...
- Per-scenario model files (no `--vars`: `dw_lib.dbt.Dbt._run_command` wraps the JSON in literal quotes, which dbt's YAML `--vars` parser rejects).
...
|---|---|---|
| "Rows without a snapshot maximum" | Non-null `DateTime64(9)` always yields a `max()`; `Nullable(...)` is rejected earlier | Comment in the validation module; no test |
| Adapter `validate_incremental_strategy` failure | With `use_lw_deletes: true` the only reachable errors need `unique_key` empty or non-`delete_insert`, both blocked by the macro first | Happy-path runs prove the call happens; note in module docstring |
...
- `clickhouse` fixture: `clickhouse_connect.get_client(host="localhost", port=18123, database="default")`.
...
- `run_model(name, *, full_refresh=False) -> ModelRun`, capturing, around one `self.dbt.run(select=name, full_refresh=...)`:
  - `log`: `logs/dbt.log` bytes appended during the run (offset captured first),
  - `queries`: `system.query_log` rows with `event_time_microseconds >=` a server timestamp taken before the run (after `SYSTEM FLUSH LOGS`), excluding the test's own query-log reads,
...
- Concurrency helper: `late_writer(...)` context manager spawning a thread with its own client that polls `system.processes` for a query like `%sleepEachRow%`, then inserts one row stamped with server `now64(9)`; it joins on exit and re-raises any timeout/insert error so the test fails loudly instead of silently passing.
...
`macros/bi_model.sql` holds the shared body (README model shape): `is_incremental()` branches, `>=` watermark by default, `order by _peerdb_version desc, _peerdb_synced_at desc limit 1 by id`, and the single `-- __BUCKET_PREDICATE__` marker in the full branch. Parameters: `watermark_operator` (`>=`/`>`), `sleep_seconds` (adds `and sleepEachRow(n) = 0` plus `settings max_threads=1` to the full branch only), `include_marker`.
...
- **`system.query_log` disabled**: helper asserts `log_queries = 1` up front and fails with a clear message.
